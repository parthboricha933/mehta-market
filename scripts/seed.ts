...
// Use Unsplash for fast-loading optimized images (with size params)
...
const PRODUCTS = [
  ...
  { name: 'Fresh Carrots', description: 'Sweet and crunchy carrots', price: 45, mrp: 55, unit: '500 g', cat: 'vegetables', img: 'photo-1598170845058-32b9d6a5da37' },
...
async function main() {
  ...
  if (!existingAdmin) {
    ...
    console.log('Admin user created (username: admin, password: mehta123)')
  }
  ...
  // Default settings
  const popupSetting = await db.setting.findFirst({ where: { key: 'offer_popup' } })
  if (!popupSetting) {
  ...
  const announcementSetting = await db.setting.findFirst({ where: { key: 'announcements' } })
  ...
  const shopSetting = await db.setting.findFirst({ where: { key: 'shop_info' } })
  if (!shopSetting) {
    await db.setting.create({
      data: {
        key: 'shop_info',
